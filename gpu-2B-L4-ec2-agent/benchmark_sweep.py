...
def discover_vllm_url(service_name="gpu-2b-qat-l4-ec2-agent"):
...
async def tokenize_prompt(url, token, prompt_text, model_name="/mnt/models/gemma-4-E2B-it-qat-w4a16-ct"):
...
async def get_prompt_for_size(url, token, size, model_name="/mnt/models/gemma-4-E2B-it-qat-w4a16-ct"):
    word = " hello"
...
async def run_sweep():
    ...
    model_name = "/mnt/models/gemma-4-E2B-it-qat-w4a16-ct"
    try:
        headers = {}
        if token:
            headers["Authorization"] = f"Bearer {token}"
