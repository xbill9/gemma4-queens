...
*   `MODEL_NAME`: The model identifier used by vLLM (defaults to `/mnt/models/gemma-4-E4B-it`). The container is started with `--model=/mnt/models/<path>`, so that path — not the Hugging Face repo id — is the name the OpenAI API expects.
*   `SERVICE_NAME`: The Cloud Run service every tool acts on unless given an explicit `service_name` (defaults to `gpu-4b-l4-devops-agent`). If you also set `VLLM_BASE_URL`, it must point at this same service.
