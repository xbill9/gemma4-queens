...
- **Default Model**: `/mnt/models/gemma-4-E4B-it` (configurable via `MODEL_NAME`) — the container is
...
- **Default Cloud Run Service Name**: `gpu-4b-l4-devops-agent` (configurable via `SERVICE_NAME`) — when
...
```bash
gcloud beta run services update gpu-4b-l4-devops-agent \
...
`--scaling=auto` with `--min-instances=0 --max-instances=1`, `--no-cpu-throttling`, and a startup probe
...
- **MCP Server entrypoint**: [server.py](file:///home/xbill/gemma4-queens/gpu-4B-cloudrun-devops-agent/server.py)
- **Deployment Manifests & Logic**: Generated by `cloudrun_get_deployment_config` and `cloudrun_get_gpu_deployment_config` in [server.py](file:///home/xbill/gemma4-queens/gpu-4B-cloudrun-devops-agent/server.py).
- **Test Suite**: [test_agent.py](file:///home/xbill/gemma4-queens/gpu-4B-cloudrun-devops-agent/test_agent.py)
- **Standalone Grand Demo**: [demo_launcher.py](file:///home/xbill/gemma4-queens/gpu-4B-cloudrun-devops-agent/demo_launcher.py)
