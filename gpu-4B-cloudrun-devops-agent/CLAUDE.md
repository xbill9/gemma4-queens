...
An MCP server (`server.py`, FastMCP) exposing SRE/DevOps tools that talk to a self-hosted
...
- `VLLM_BASE_URL` is optional — when unset, `discover_vllm_url()` finds the service via `gcloud`.
  ...
  defaults mirror the `Makefile`, so update both together.
...
(`config://vllm-deployment-template`); new resources go in `test_resources_registered` the same way.
