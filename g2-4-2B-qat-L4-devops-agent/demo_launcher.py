...
async def devops_demo():
    ...
    analysis = await gce_analyze_cloud_logging(filter_query="severity=ERROR", limit=2)
    ...
    remediation = await gce_suggest_sre_remediation(error_message="Pod 'vllm-gemma' terminated with Reason: OOMKilled")
    print(f"  REMEDIATION: {remediation}")

    # Step 3: Deployment Config & Vertex AI instructions
    print("\n[Step 3] Vertex AI Model Garden Instructions...")
    instructions = gce_get_vertex_ai_model_copy_instructions(model_name="gemma-4-E2B-it-qat-w4a16-ct")
    print(instructions)

    print("\n[Step 4] Generating GCE g2-standard-4 (NVIDIA L4) vLLM Deployment Config...")
    config = gce_get_vllm_deployment_config(
        instance_name="vllm-sre-agent",
