...
mcp = MCPServer("tpu-12B-v6e1-devops-agent")
...
# The only VPC in aisprint-491218 is `default`; an earlier hardcoded `vpc-glitnir` no
# longer exists and made every create fail with "provided for field Network does not exist".
TPU_NETWORK = os.getenv("TPU_NETWORK", "default")
TPU_SUBNETWORK = os.getenv("TPU_SUBNETWORK", TPU_NETWORK)
# gcloud flags per provisioning model. on-demand is the API default, so it passes no flag;
# its lack of --max-run-duration is the point — it holds until deleted instead of expiring.
PROVISIONING_MODEL_FLAGS = {
    "flex-start": ["--provisioning-model=flex-start", "--max-run-duration=4h", "--valid-until-duration=4h"],
    "on-demand": [],
    "reserved": ["--reserved"],
}
...
async def _get_formatted_startup_script(model_name: str) -> str:
    """Formats the startup script with necessary values.

    The HF token is deliberately not interpolated here: the script fetches it from
...
async def get_vllm_deployment_config(service_name: str = "vllm-gemma4-qr", model_name: str = MODEL_NAME) -> str:
    ...
    cmd = (
        ...
        f"--max-model-len 8192 --gpu-memory-utilization 0.95 "
...
async def manage_queued_resource(
    resource_id: str = "vllm-gemma4-qr", zone: str = ZONE, provisioning_model: str = "flex-start"
) -> str:
    """Ensures the primary Queued Resource exists and cleans up redundant ones.

    provisioning_model is one of 'flex-start' (default: discounted, 4h max-run, expires),
    'on-demand' (no run limit, bills until deleted), or 'reserved' (needs a capacity
    reservation in the project — fails outright if none exists).
    """
    # Validate before the cleanup loop below deletes anything.
    if provisioning_model not in PROVISIONING_MODEL_FLAGS:
        valid = ", ".join(sorted(PROVISIONING_MODEL_FLAGS))
        return f"❌ Unknown provisioning_model '{provisioning_model}'. Expected one of: {valid}."
    ...
    if not primary_res:
        # Presence check only — the node fetches the token itself at boot.
        if not await get_secret():
            return "❌ Aborted: 'hf-token' secret missing."

        startup_script_content = await _get_formatted_startup_script(MODEL_NAME)
        ...
        create_cmd = [
            ...
            *PROVISIONING_MODEL_FLAGS[provisioning_model],
            f"--project={PROJECT_ID}",
            f"--labels=purpose={provisioning_model}",
            f"--accelerator-type={ACCELERATOR_TYPE}",
            f"--metadata-from-file=startup-script={script_file}",
            f"--network={TPU_NETWORK}",
            f"--subnetwork={TPU_SUBNETWORK}",
        ]
...
async def create_tpu_queued_resource(
    ...
    """Creates a TPU Queued Resource in the given zone.

    provisioning_model: 'flex-start' (default: discounted, 4h max-run, then expires),
    'on-demand' (no run limit, bills until deleted), or 'reserved' (requires an existing
    capacity reservation).
    """
    return await manage_queued_resource(resource_id=resource_id, zone=zone, provisioning_model=provisioning_model)
...
async def manage_vllm_docker(resource_id: str = "vllm-gemma4-qr", action: str = "start") -> str:
    ...
    docker_run_cmd = (
        ...
        # Keep these two values in sync with startup_script_template.sh. The 12B weights take ~24GB of
        # the v6e-1's 32GB HBM, so the KV cache has to live in what is left — a longer context, or a
        # lower utilization, fails to allocate at startup.
