...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("server._get_formatted_startup_script", new_callable=AsyncMock)
    @patch("server.get_secret", new_callable=AsyncMock)
    @patch("server.run_command", new_callable=AsyncMock)
    async def test_create_tpu_queued_resource_provisioning_models(
        self, mock_run_command, mock_get_secret, mock_startup_script
    ):
        """Each provisioning model maps to the right gcloud flags."""
        mock_get_secret.return_value = "dummy-hf-token"
        mock_startup_script.return_value = "#!/bin/bash\necho hi"

        cases = {
            "flex-start": ["--provisioning-model=flex-start", "--max-run-duration=4h"],
            "on-demand": [],
            "reserved": ["--reserved"],
        }
        for model, expected_flags in cases.items():
            with self.subTest(provisioning_model=model):
                # First call lists (empty), second creates.
                mock_run_command.reset_mock()
                mock_run_command.return_value = 0, "[]", ""
                await create_tpu_queued_resource(resource_id="qr-test", provisioning_model=model)

                create_cmd = mock_run_command.call_args_list[-1].args[0]
                for flag in expected_flags:
                    self.assertIn(flag, create_cmd)
                # Flags belonging to the other models must not leak in.
                for flag in set().union(*cases.values()) - set(expected_flags):
                    self.assertNotIn(flag, create_cmd)
                self.assertIn(f"--labels=purpose={model}", create_cmd)

    @patch("server.run_command", new_callable=AsyncMock)
    async def test_create_tpu_queued_resource_rejects_unknown_model(self, mock_run_command):
        """An unknown provisioning model is rejected before anything is deleted."""
        result = await create_tpu_queued_resource(resource_id="qr-test", provisioning_model="spot")
