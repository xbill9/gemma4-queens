...
This is a live-demo DevOps/SRE agent for serving `google/gemma-4-12B-it` with vLLM on a Google Cloud TPU
v6e-1 Flex-start Queued Resource. The main application is a single-file MCP server (`MCPServer`) in `server.py`. Its
...
- The effective zone and region are currently hardcoded near the top of `server.py`. Values in
  `mcp_config.json` and Makefile defaults do not override those literals.

## Code conventions

- Target Python 3.10. Ruff is the formatter and linter; do not introduce Black or a separate isort setup.
...
- `startup_script_template.sh` is rendered with Python `str.format()`. Its supported placeholders include
  `{project_id}`, `{zone}`, `{model_name}`, `{hf_secret_id}`, `{tensor_parallel_size}`, and
  `{limit_mm_per_prompt_env}`. Escape every other literal brace as `{{` or `}}`, including shell `${VAR}` and
  JSON braces, or deployment rendering will fail.
...
- Environment variables actually consumed at startup include `GOOGLE_CLOUD_PROJECT`, `MODEL_NAME`,
  `ACCELERATOR_TYPE`, `TENSOR_PARALLEL_SIZE`, and `LOCAL_DOCKER_IMAGE`. Confirm the source before documenting
